...
N = 1000
#c0_vec = [1 / 32]
c0_vec = [1 / 32, 1/16, 1/8, 1/4, 1/2] #1 / 32, 1/16, 1/8,
Algo_num = 1
...
def run(c0):
	...
	rlb_dp_i.alpha_tune(src, camp, camp_info, m_pdf, const_risk_tendency, camp_info['data'], bid_log_path, N, B, c0,
						max_market_price, clk_stat_interval)


	rlb_dp_i.calc_risk_tendency(N, B, max_market_price, m_pdf, const_risk_tendency, True, ifconst_risktendency=False)

	(auction, imp, clk, return_ctr, return_risk, cost, clk_stat) = rlb_dp_i.run(auction_in['data'], bid_log_path, N,
																					  B,
																					  max_market_price, clk_stat_interval,
																					  save_log=True,

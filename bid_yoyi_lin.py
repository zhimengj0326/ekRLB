...
def run(c0):
	...
	model_path = config.ipinyouPath + "bid_log/{}_{}_{}_{}_{}.pickle".format("lin-bid", N, B, c0, obj_type, opt_obj.clk_v)
	...
	lin_bid.parameter_tune(opt_obj, camp_info['data'], model_path, N, B, c0, int(max_market_price / 100), max_market_price, clk_stat_interval, load=False)
	(auction, imp, clk, return_ctr, cost, clk_stat) = lin_bid.run(auction_in['data'], bid_log_path,
...
def main():
	...
	clk_stat_tot = np.zeros([np.ceil(max_market_price / clk_stat_interval).astype(int), len(c0_vec), Algo_num])

...
def argsparser():
    ...
    return parser
def test_performance(sess, camp, c0, max_bid, rtb_environment, m_pdf, mlp_network, episodes_n, init_budget, test_file, clk_stat_interval):
    ## Test
    # DQN
    risk_tendency_table2 = risktendency_get(sess, mlp_network, episodes_n, init_budget)

    path = "results/camp={}_c0={}_risk_tendency_learn.pdf".format(camp, c0)
...
def run(camp, src, buffer_time, train_steps, batch_size, episodes_n, episodes_num, state_size, action_size, learning_rate, hidden_num, hidden_unit, c0, max_bid, clk_stat_interval):
    ...
    with tf.compat.v1.Session() as sess:
        ...
        path = "results/camp={}_c0={}_risk_tendency_ori.pdf".format(camp, c0)
        ...
        for i in range(train_ini_steps):
            ...
            if global_step_counter % trace_solu ==0:
                (_, _, clk, _, _) = test_performance(sess, camp, c0, max_bid, rtb_environment, m_pdf,
                                                                       mlp_network, episodes_n, init_budget, test_file,
                                                                       clk_stat_interval)
                clk_trace.extend(clk)
        ...
        path = "results/camp={}_c0={}_risk_tendency_ini_learn.pdf".format(camp, c0)
        plot_risk_tendency(risk_tendency_table, path)
        ...
        ## Test
        # DQN
        (auction, imp, clk, cost, clk_stat) = test_performance(sess, camp, c0, max_bid, rtb_environment, m_pdf, mlp_network, episodes_n, init_budget, test_file, clk_stat_interval)
        clk_trace.extend(clk)
        print("the trace of click number".format(clk_trace))
    sess.close()
    return auction, imp, clk, cost, clk_stat, clk_trace
...
def main():
    ...
    auction_tot, imp_tot, click_tot, cost_tot, clk_stat_tot, clk_trace_tot = zip(*Parallel(processNum, 'multiprocessing', max_nbytes=None)(
		delayed(run)(camps[camp_index], src, buffer_time, train_steps, batch_size, episodes_n, episodes_num, state_size, action_size, learning_rate, hidden_num, hidden_unit, c0_vec[c0_index], max_bid, clk_stat_interval)
        for camp_index in range(len(camps)) for c0_index in range(len(c0_vec))))

    click_tot = np.array(click_tot).reshape((len(camps), len(c0_vec)))
    auction_tot = np.array(auction_tot).reshape((len(camps), len(c0_vec)))
    imp_tot = np.array(imp_tot).reshape((len(camps), len(c0_vec)))
    cost_tot = np.array(cost_tot).reshape((len(camps), len(c0_vec)))
    ...
    results_file = "results/ssRTB_results.pickle"
    with open(results_file, 'wb') as f:
        ssRTB_results = {'auction_tot': auction_tot, 'imp_tot': imp_tot, 'cost_tot': cost_tot, 'click': click_tot}
    ...
    results_file = "results/ssRTB_avg_results.pickle"
    with open(results_file, 'wb') as f:
